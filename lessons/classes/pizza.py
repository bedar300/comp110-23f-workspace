...
"""
Think of a class as a definion as a
'roadmap' for objects that belong to the class
You are defnining the underlying structure every 
...
class Pizza:
    ...
    def price(self) -> float:
        """Methoss to compute price of Pizza"""
        if self.size == "large":
            cost: float = 6.25
        else:
            cost: float = 5.00
    ...
    def add_toppings_new_order(self, num_toppings: int) -> Pizza:
        """return a new pizza order with num_toppings"""
